...
// Define weights for cost evaluation
const double cte_weight = 2000;
const double epsi_weight = 2000;
const double v_weight = 3;
const double delta_weight = 5;
...
class FG_eval {
 public:
  ...
  FG_eval(Eigen::VectorXd coeffs, double dt, double Lf) {
    this->coeffs = coeffs;
    this->dt = dt;
    this->Lf = Lf;
  }
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      // Consider the actuation at time t (related to latency)
